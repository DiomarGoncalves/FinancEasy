...
async function obterOuCriarFatura(cartao_id) {
    ...
    if (!faturaAtual) {
        const hoje = new Date();
        const data_inicio = new Date(hoje.getFullYear(), hoje.getMonth(), 1).toISOString().split('T')[0];
        const data_fim = new Date(hoje.getFullYear(), hoje.getMonth() + 1, 0).toISOString().split('T')[0];
...
ipcMain.handle("criar-fatura", async (event, cartao_id, data_inicio, data_fim) => {
                    ...
                    if (err) {
                        ...
                        db.run(deleteSql, [cartao_id], function (err) {
                            if (err) {
                                ...
                                db.run(updateSql, [cartao_id, data_pagamento, cartao_id], function (err) {
                                    if (err) {
                                        reject(err);
                                    } else {
                                        resolve({ changes: this.changes });
